...
// @downloadURL https://update.greasyfork.org/scripts/39292/AO3%3A%20Reorder%20Fandoms%20by%20Size.user.js
// @updateURL https://update.greasyfork.org/scripts/39292/AO3%3A%20Reorder%20Fandoms%20by%20Size.meta.js
// ==/UserScript==
	...
	if (alphabetGroup[0] != undefined && alphabetGroup[0].classList.contains("sort-by-alphabet") == false) {
		...
		for (var i = 0; i < numericalList.length; i++) {
			...
			try {
				...
				console.error(`lol. no fandomSizeRanges[x] i bet.\n`, e);
			}
...
button.innerHTML = `<a href="#">Sort Fandoms By Size</a>`;
button.addEventListener("click", reorder);
document.querySelector(`#migrations-helper`).appendChild(button);